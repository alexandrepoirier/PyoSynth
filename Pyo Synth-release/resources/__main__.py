...
class PyoSynth(wx.Frame):
    def __init__(self, server, namespace, chnl=0, poly=10, numControls=16):
        ...
        self.Bind(wx.EVT_MENU, self.menu_panel._savePreset, id=100)
        openitem = filemenu.Append(101, "Open Script\tCtrl+O","",wx.ITEM_NORMAL)
        self.Bind(wx.EVT_MENU, self.menu_panel._openDialog, id=101)
        openrecent = wx.Menu()
        ...
        self.Bind(wx.EVT_MENU, self.menu_panel.exc_win.toggle, id=104)
    ...
    def setPreset(self, preset):
        ...
        for key in preset:
            ...
            try:
                ...
                self.menu_panel.exc_win.printException(self.menu_panel._script_path.split('/')[-1], "PresetError : "+str(e))
            else:
                val = preset[key]['val']
                if val != None:
                    valuesDict[key] = val
